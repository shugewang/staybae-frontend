...
const Search = () => {
  ...
  useEffect(() => {
    const findPropertiesForCriteria = async () => {
      ...
      try {
        const response = await Axios.get(`/properties?from=${fromDate}&to=${toDate}&city=${searchLocation}&region=${region}`);
  ...
  return (
    <div className='flex pt-10'>
      <section className="flex-grow px-6">
        <p className="text-xs">{searchResults.length} Stays - {range} - for {numberOfGuests} guest(s)</p>
